...
preds1=pd.read_csv(inputdir+'XGBOOST_Best_score_0.818532.csv.gz')
...
#print preds0['predict_screener']
preds=(0.45*preds0['predict_screener'].values+0.55*preds1['predict_screener'].values)

idx = preds0.patient_id.values.astype(int)

# Create your first submission file
submission = pd.DataFrame({"patient_id": idx, "predict_screener": preds})
submission.to_csv(inputdir+"Multi-model-VW-XGB-best.csv", index=False)
