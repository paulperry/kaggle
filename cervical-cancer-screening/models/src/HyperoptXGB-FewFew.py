...
train_physician_family=pd.read_csv('../features/train_physician_family.csv.gz')
test_physician_family=pd.read_csv('../features/test_physician_family.csv.gz')
train=pd.merge(train,train_physician_family,on='patient_id',how='left')
test=pd.merge(test,test_physician_family,on='patient_id',how='left')
print('after merging physician family')
print(train.shape,test.shape)
del train_physician_family,test_physician_family
...
train =train.drop([u'RX_ASSISTANCE', u'RX_CASH', u'RX_COMMERCIAL', u'RX_MANAGED_MEDICAID', u'RX_MEDICAID', u'RX_MEDICARE'],axis=1)

test =test.drop([u'RX_ASSISTANCE', u'RX_CASH', u'RX_COMMERCIAL', u'RX_MANAGED_MEDICAID', u'RX_MEDICAID', u'RX_MEDICARE'],axis=1)
...
clf = xgb.XGBClassifier(n_estimators = 3000,subsample=best['subsample'],colsample_bytree=best['colsample_bytree'],colsample_bylevel=best['colsample_bylevel'],
                        ...
                        min_child_weight=best['min_child_weight'],max_depth=best['max_depth'],seed=19)
eval_set  = [( d_train, y_train), ( d_val, y_val)]
