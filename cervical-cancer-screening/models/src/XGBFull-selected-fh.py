...
fh=feature_extraction.FeatureHasher(n_features=1024, input_type='string',non_negative=False)
X=np.asarray(train).astype(str)
y=np.asarray(y)
X_test=np.asarray(test).astype(str)
...
X_val=fh.transform(X_val)
X_test=fh.transform(X_test)
X_val0=fh.transform(X_val0)
